...
local function loadSave()
	...
	if ok and result then
		for k,v in pairs(DEFAULT_SAVE) do if result[k]==nil then result[k]=v end end
		return result
	end
	return DEFAULT_SAVE
end
...
KeyOverlayBg.Size=UDim2.new(1,0,1,0); KeyOverlayBg.BackgroundTransparency=1
KeyOverlayBg.Image=ID_ANH_NEN; KeyOverlayBg.ImageTransparency=0.55
...
AvatarCardBg.Size=UDim2.new(1,0,1,0); AvatarCardBg.BackgroundTransparency=1
AvatarCardBg.Image=ID_ANH_NEN; AvatarCardBg.ImageTransparency=0.78
AvatarCardBg.ScaleType=Enum.ScaleType.Crop; AvatarCardBg.ZIndex=3; AvatarCardBg.Parent=AvatarCard; corner(AvatarCardBg,10)
...
AvatarImg.Size=UDim2.new(0,54,0,54); AvatarImg.Position=UDim2.new(0.5,-27,0,10)
...
DName.Size=UDim2.new(1,-6,0,18); DName.Position=UDim2.new(0,3,0,68)
...
UName.Size=UDim2.new(1,-6,0,14); UName.Position=UDim2.new(0,3,0,86)
...
ADiv.Size=UDim2.new(1,-16,0,1); ADiv.Position=UDim2.new(0,8,0,104)
...
UIDLabel.Size=UDim2.new(1,-6,0,13); UIDLabel.Position=UDim2.new(0,3,0,108)
...
UpdateCardBg.Size=UDim2.new(1,0,1,0); UpdateCardBg.BackgroundTransparency=1
UpdateCardBg.Image=ID_ANH_NEN; UpdateCardBg.ImageTransparency=0.85
...
UpdateTitle.Size=UDim2.new(1,-10,0,20); UpdateTitle.Position=UDim2.new(0,7,0,5)
...
UDiv.Size=UDim2.new(1,-12,0,1); UDiv.Position=UDim2.new(0,6,0,25)
...
UpdateScroll.Size=UDim2.new(1,-6,1,-28); UpdateScroll.Position=UDim2.new(0,3,0,27)
...
for _,u in ipairs(updates) do
	...
	local ver=Instance.new("TextLabel"); ver.Size=UDim2.new(0,32,1,0); ver.Position=UDim2.new(0,3,0,0)
	...
	local desc=Instance.new("TextLabel"); desc.Size=UDim2.new(1,-36,1,0); desc.Position=UDim2.new(0,34,0,0)
		...
		for _,plr in ipairs(Players:GetPlayers()) do
			if plr~=LocalPlayer then
				...
				if char and bp then
					...
					if st=="strong" then
						if last~="strong" then createBillboard(char,"💢") end; skillDetectorState[plr]="strong"
					elseif st=="weak" then
						if last=="strong" then
						...
						else removeBillboard(char) end; skillDetectorState[plr]="weak"
...
local function stopSkillDetector()
	if skillDetectorConn then skillDetectorConn:Disconnect(); skillDetectorConn=nil end; clearAllBillboards()
end
...
TabBar.Size=UDim2.new(1,-(TAB_H+GAP),1,0); TabBar.Position=UDim2.new(0,0,0,0)
...
TabScroll.Size=UDim2.new(1,-6,1,-2); TabScroll.Position=UDim2.new(0,3,0,1)
...
CloseBtn.Font=Enum.Font.GothamBold; CloseBtn.TextSize=13; CloseBtn.ZIndex=6; CloseBtn.Parent=TabRow; corner(CloseBtn,9)
...
ContentBgImg.Size=UDim2.new(1,0,1,0); ContentBgImg.BackgroundTransparency=1
...
local function animateClose(cb)
	tw(Root,{Size=UDim2.new(0,ROOT_W*0.88,0,ROOT_H*0.88),Position=UDim2.new(0.5,-(ROOT_W*0.88)/2,0.5,-(ROOT_H*0.88)/2),BackgroundTransparency=1},0.25,Enum.EasingStyle.Quint,Enum.EasingDirection.In)
...
local function slideContent(newIndex,loadFunc)
	...
	tw(ContentFrame,{Position=UDim2.new(0,exitX,0,4)},0.16,Enum.EasingStyle.Cubic,Enum.EasingDirection.In)
	task.wait(0.17)
	...
	tw(ContentFrame,{Position=UDim2.new(0,3,0,4)},0.20,Enum.EasingStyle.Cubic,Enum.EasingDirection.Out)
	task.wait(0.21); isSliding=false
end
...
local function makeTab(name)
	local btn=Instance.new("TextButton"); btn.Size=UDim2.new(0,66,0,24); btn.Text=name
...
local function makeColorBtn(label,color,onClick)
	...
	local lbl=Instance.new("TextLabel"); lbl.Size=UDim2.new(0.68,0,1,0); lbl.Position=UDim2.new(0,28,0,0)
...
local function LoadPlayer()
	...
	makeToggle("👻  Noclip",false,function(v)
		toggleNoclip(v)
		task.spawn(function() showToast(v and "👻 Noclip BẬT" or "👻 Noclip TẮT",v and C.GREEN or C.RED,2) end)
	end)
	makeToggle("🤖  Anti-AFK",false,function(v)
		toggleAntiAFK(v)
...
local function LoadServer()
	...
	makeActionBtn("Hop Server (Random)","🔀",function()
		...
		if ok and sv and sv.data then
			...
			if #list>0 then task.spawn(function() showToast("🔀  Đang hop...",C.CYAN,2) end); task.wait(1)
...
PlayerTab.MouseButton1Click:Connect(function()  setActive(PlayerTab,5,LoadPlayer) end)
